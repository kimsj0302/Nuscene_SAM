...
imageset_trainval = os.path.join(sample_pkl_path, "nuscenes_infos_train.pkl")
...
for index in trange(int(num_totdata*st_range),int(num_totdata*fn_range)):
    ...
    for idx, channel in enumerate(CAM_CHANNELS):
        ...
        # breakpoint()
        torch.save(img_fea.half(), os.path.join(dst_img,channel,lidar_path.split('/')[-1].split('.')[0]+".pt"))
        torch.save(final_sam.half(), os.path.join(dst_seg,channel,lidar_path.split('/')[-1].split('.')[0]+".pt"))
# breakpoint()
